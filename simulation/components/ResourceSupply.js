...
ResourceSupply.prototype.GetEnroute = function(player)
{
	if (player === undefined)
		return this.enroute;
	return this.enroute[player];
};
...
{
	var numOfGatherers = this.GetGatherers().length + this.GetEnroute(player).length;
...
ResourceSupply.prototype.AddEnrouteGatherer = function(player, gathererID)
{
	...
	/*	Engine.PostMessage(this.entity, MT_ResourceSupplyGatherersEnrouteChanged, { "to": this.GetEnroute(player) });	*/
	}
